...
function buildMsa(msa, csaId) {
  report.msas++;
  const existingId = titleToExisting[msa.name];
...
// ---- merge: keep existing curated (international + the 18) then new ------
// existing entries retain their position; new official MSAs appended.
const mergedMetros = [...existingDoc.metroAreas, ...newMetros];
// de-dupe by id (existing 18 already mutated in place; new ones have fresh slugs)
const seen = new Set(); const finalMetros = [];
for (const m of mergedMetros) { if (seen.has(m.id)) continue; seen.add(m.id); finalMetros.push(m); }

existingDoc.metroAreas = finalMetros;
